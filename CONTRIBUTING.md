...
- **Live implementation**: `LiveNetwork` uses `URLSession.shared` for production. It retries transient transport failures (timeout, dropped or missing connection — `URLError.isTransientTransportFailure`) with a short exponential backoff, so one blip doesn't fail a refresh; HTTP statuses are never retried (a 429 must be honored with a pause, not hammered). `Trafiklab` checks the HTTP status before decoding and throws `TrafiklabHTTPError(statusCode:path:)` for a non-2xx response — the error carries only the status and the request's path, never the query, which holds the API key.
