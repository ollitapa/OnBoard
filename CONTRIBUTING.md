...
- **API keys come from a bundled env file, not source**: `Trafiklab` reads its keys through `Secrets`, which parses the `KEY=VALUE` env file bundled with the app: `OnBoard/Secrets.env` (git-ignored, your real keys) with the committed `OnBoard/Secrets.example.env` (bogus keys) as the fallback so the project builds with no setup. To use real keys, copy the example file to `OnBoard/Secrets.env` and fill in your keys from developer.trafiklab.se — the file-system-synchronized target bundles both files as resources automatically. The key accessors (`Secrets.realtimeKey`/`resrobotKey`) are throwing computed properties: a missing or empty key throws `SecretsError.missingKey` when a request is built, so it surfaces through the view model's `failure` handling instead of silently sending an empty key.
