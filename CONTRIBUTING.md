...
- **Prefer feature-specific service mocks over a bare `MockNetwork`**: `MockTrafiklabService` (and the older `MockNearbyServer`) serve canned responses for the real endpoint paths the app calls, so a model's full production decode path is exercised. Configure `MockTrafiklabService` with JSON strings in the endpoint's wire shape (`nearbyStops` as `[String]`; `departuresByAreaId`/`tripsByKey`/`stopGroupsByName` as `[String: String]`), not hand-built API model values — a fixture should read like a captured API response, and it is served by joining the chosen strings into the response envelope, never converted into the API's Swift models. The Stop Lookup name search matches the dictionary keys (each key is the group's name, repeating the name inside the JSON is fine), so the JSON strings are never searched; served groups come back in name order, not the real API's busiest-first order. Timestamps inside a fixture may use relative markers (`"now"`, `"now+2"`, `"now-10"`, in minutes) resolved at creation so previews and UI tests always show fresh times; malformed fixture JSON traps with the fixture named. Reach for the bare `MockNetwork` (handler registration) only when a test needs to assert on a specific request shape or feed invalid JSON (`loadStopsInvalidJSON`, `loadDeparturesInvalidJSON`). A preview that wires a `MockNetwork()` with **no handlers** into a screen whose `.task` loads data will surface `NoResponseConfigured` as a failure string, not a working preview — use `MockTrafiklabService` (or `mockNetwork()`) for previews that need data.
...
- **Write mock fixtures as the JSON you'd capture from the real endpoint, not the Swift values you'd construct.** This is the distilled guideline behind `MockTrafiklabService`'s design, learned by removing code that re-implemented things the app already had:
  - A fixture is a multiline JSON string in the endpoint's wire shape, keyed by what the request identifies (`areaId`, `"{tripId}/{startDate}"`, group name). Paste a captured response in verbatim; never hand-build API model values in the mock layer — that makes the mock a second, drifting implementation of the endpoint.
  - The serving path does no decoding or encoding of the fixture: pick the string by key, join the chosen strings into the response envelope, serve `Data(body.utf8)`. The model under test then exercises its real production `Codable` decode on exactly the fixture bytes, so a broken fixture fails as a model failure — where you want it — not as a mock bug.
  - Key dictionaries by the thing you look up, even if a value repeats inside the JSON (`stopGroupsByName` repeats the name in key and value). A dictionary lookup replaces any searching, substring matching, or range scanning inside JSON strings, which is fragile.
  - Dictionary fixtures are unordered, so give the served result an explicit, documented order — the mock serves name-sorted stop groups and documents that deviation from the real API's busiest-first order. And a test that reads expected values back can decode the stored fixture with `JSONDecoder` instead of duplicating literals.
  - Resolve time-relative data once at creation: relative timestamp markers (`"now"`, `"now+2"`, `"now-10"`) become absolute timestamps when the service is built, so previews and UI tests always show fresh times with no per-request recomputation. Validate every fixture at the same moment — a malformed one traps with the fixture named rather than silently serving an empty response later.
  - Parse request paths with small regex captures (`url.path.firstMatch(of: #/departures/([^/]+)/#)`) instead of split-and-index arithmetic, and prefer `String.replacing(_:with:)` with a capture-group regex over a manual scan-and-rebuild loop for textual substitution; both read as the endpoint's shape rather than as index bookkeeping.
...
- **User-facing text goes through the string catalog**: All in-app strings — view `Text`, `Button` titles, labels — live in `OnBoard/Localizable.xcstrings` (English source with Swedish and Finnish translations) and are referenced through the Xcode-generated `LocalizedStringResource` symbols, e.g. `Text(.stopBoardEmptyTitle)`; the `STRING_CATALOG_GENERATE_SYMBOLS = YES` build setting makes those symbols compile-checked. Add a key with a dotted, PascalCase-per-segment name (`StopBoard.EmptyTitle`), a translator comment, and the three translations; Xcode camel-cases the key into the symbol (`.stopBoardEmptyTitle`), and named placeholders (`%(minutes)lld`) become typed function parameters (`.departureMinutes(minutes: 3)`). The Trafiklab API is Swedish and its data (stop names, etc.) is surfaced as-is, outside the catalog. Runtime-built strings (a model's `failure` text) are not localized — pass them to a `Text`-typed slot with `Text(verbatim:)`. The `NSLocationWhenInUseUsageDescription` Info.plist value is the *system* permission prompt, not in-app UI, so it may be localized to match the OS sheet. Unit and UI tests pin `-AppleLanguages (en)` so their English string expectations are locale-proof on any runner.
